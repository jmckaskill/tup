...
int parse(struct node *n, struct graph *g)
{
	...
	if(n->parsing) {
		fprintf(stderr, "Error: Circular dependency found among Tupfiles (last dir ID %"PRI_TUPID"  = '%s').\nThis is madness!\n", n->tnode.tupid, n->tent->name.s);
		return -1;
	}
	n->parsing = 1;
		...
		if(strcmp(line, "else") == 0) {
			...
			tupid_tree_for_each(symid, rbn, &symtree) {
				...
				if(tent->type == TUP_NODE_GENERATED) {
					fprintf(stderr, "Error: Attempted to include a Tupfile from a path that contains a symlink generated by tup. Directory %"PRI_TUPID", symlink used was %"PRI_TUPID", line number %i\n", tf->tupid, tent->tnode.tupid, lno);
					sym_bork = 1;
				}
			}
			if(sym_bork)
...
syntax_error:
	fprintf(stderr, "Error parsing Tupfile [%"PRI_TUPID"] line %i\n  Line was: '%s'\n", tf->tupid, lno, line);
	...
	list_for_each_entry_safe(struct name_list_entry, nle, tmpnle, &nl->entries, list) {
		...
		if(nle->tent->type != TUP_NODE_FILE) {
			if(nle->tent->type == TUP_NODE_GENERATED) {
				...
				fprintf(stderr, "tup error: Attempt to include node (ID %"PRI_TUPID", name='%s') of type %i?\n", nle->tent->tnode.tupid, nle->path, nle->tent->type);
				return -1;
			}
		}
			       ...
			       int lno, int required)
{
	...
	tupid_tree_for_each(symid, rbn, &symtree) {
		tent = tup_entry_get(symid);
		if(tent->type == TUP_NODE_GENERATED) {
			fprintf(stderr, "Error: Attempted to input files using a symlink that was generated by tup. Directory %"PRI_TUPID", symlink used was %"PRI_TUPID", line number %i\n", tf->tupid, tent->tnode.tupid, lno);
			sym_bork = 1;
		}
	}
	if(sym_bork)
	...
	do {
		...
		if(p[0] == '{') {
			...
			*endb = 0;
			...
			if(pl->dt <= 0) {
				fprintf(stderr, "Error: Failed to find directory ID for dir '%s' relative to %"PRI_TUPID"\n", p, dt);
				return -1;
			}
		       ...
		       struct name_list *nl, int required)
{
	...
	if(char_find(pl->pel->path, pl->pel->len, "*?[") == 0) {
		...
		if(!tent) {
			if(!required)
				return 0;
			fprintf(stderr, "Error: Explicitly named file '%.*s' not found in subdir %"PRI_TUPID".\n", pl->pel->len, pl->pel->path, pl->dt);
		...
		if(tupid_tree_search(&tf->g->delete_tree, tent->tnode.tupid) != NULL) {
			if(!required)
				return 0;
			fprintf(stderr, "Error: Explicitly named file '%.*s' in subdir %"PRI_TUPID" is scheduled to be deleted (possibly the command that created it has been removed).\n", pl->pel->len, pl->pel->path, pl->dt);
			tup_db_print(stderr, pl->dt);
			return -1;
		}
		if(build_name_list_cb(&args, tent) < 0)
		   ...
		   struct name_list *output_nl)
{
	...
	if(tupid_tree_insert(&tf->cmd_tree, cmd_tt) < 0) {
		fprintf(stderr, "Error: Attempted to add duplicate command ID %"PRI_TUPID"\n", cmdid);
