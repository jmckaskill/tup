...
#ifdef _WIN32
#define LDPRELOAD_FORMAT "%s\\tup-inject.dll"
#else
#define LDPRELOAD_FORMAT "%s/tup-ldpreload.so"
#endif

#ifdef _WIN32
static WSADATA s_winsock;
#endif
...
int server_init(void)
{
	int ret = snprintf(
			ldpreload_path,
			sizeof(ldpreload_path),
			LDPRELOAD_FORMAT,
			getexecwd());

	if (ret >= (signed)sizeof(ldpreload_path)) {
...
#ifdef _WIN32
	char fd_name[32];
	snprintf(fd_name, sizeof(fd_name), "%d", s->udp_port);
	fd_name[31] = 0;
	setenv(TUP_SERVER_UDP_PORT, fd_name, 1);
#else
...
#ifndef _WIN32
#define TUP_SOCK ".tup/sock"

static int connect_unix(socket_t socks[2])
{
	/* TODO */
	struct sockaddr_un sa;

	if (unlinkat(tup_top_fd(), TUP_SOCK)) {
...
#ifdef _WIN32
	WSAStartup(MAKEWORD(2,2), &s_winsock);
